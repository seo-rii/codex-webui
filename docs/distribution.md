...
Startup keeps runtime profile state lightweight. The gateway restores persisted queue/shutdown metadata for every profile, but it does not start Codex app-server processes until a profile receives an active Codex request. `CODEX_WEBUI_MAX_APP_SERVERS` caps concurrent Codex app-server processes and defaults to `1`; `CODEX_WEBUI_APP_SERVER_TIMEOUT_SECONDS` defaults to `600` so long-session `thread/read`, resume, and `turn/start` requests are not cut off during initial loading; `CODEX_WEBUI_SERVER_THREADS`, `CODEX_WEBUI_BLOCKING_THREADS`, `CODEX_WEBUI_SERVER_THREAD_STACK_BYTES` (default and minimum `16777216`), and `CODEX_WEBUI_CONTROLLER_THREADS` can be raised for larger hosts. On constrained code-server hosts, reduce `CODEX_WEBUI_SERVER_THREADS` and `CODEX_WEBUI_BLOCKING_THREADS` before lowering stack size; too small a stack can crash deep session and JSON processing.
